<?php
/**
 * Vw40AtencaoMedicaList Listing
 * @author  <your name here>
 */
class Vw40AtencaoMedicaList extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_Vw40AtencaoMedicaPrimerissimaInfancia');
        $this->form->setFormTitle('A40 - Cunhantãs e Curumins novinhos que não foram ao médico');
        ...
        $this->form->setData( TSession::getValue('Vw40AtencaoMedicaPrimerissimaInfancia_filter_data') );
        
        // add the search form actions
        $btn = $this->form->addAction(_t('Find'), new TAction([$this, 'onSearch']), 'fa:search');
        $btn->class = 'btn btn-sm btn-primary';
        //$this->form->addActionLink(_t('New'), new TAction(['Vw40AtencaoMedicaPrimerissimaInfanciaForm', 'onEdit']), 'fa:plus green');
        ...
        // create EDIT action
        //$action_edit = new TDataGridAction(['Vw40AtencaoMedicaPrimerissimaInfanciaForm', 'onEdit']);
...
##LIST_DELETE_ACTION##
        ...
        $tipo='A40 - Cunhantãs e Curumins novinhos que não foram ao médico';
...
##LIST_FORM_COLLECTION##
    ...
    public function onInlineEdit($param)
    {
        try
        {
            ...
            $object = new Vw40AtencaoMedicaPrimerissimaInfancia($key); // instantiates the Active Record
            $object->{$field} = $value;
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('Vw40AtencaoMedicaList_filter_pessoa_id',   NULL);
        TSession::setValue('Vw40AtencaoMedicaList_filter_nome',   NULL);
        TSession::setValue('Vw40AtencaoMedicaList_filter_mae',   NULL);
        TSession::setValue('Vw40AtencaoMedicaList_filter_cns',   NULL);
        TSession::setValue('Vw40AtencaoMedicaList_filter_idade',   NULL);
        TSession::setValue('Vw40AtencaoMedicaList_filter_qtd',   NULL);

        if (isset($data->pessoa_id) AND ($data->pessoa_id)) {
            $filter = new TFilter('pessoa_id', '=', "$data->pessoa_id"); // create the filter
            TSession::setValue('Vw40AtencaoMedicaList_filter_pessoa_id',   $filter); // stores the filter in the session
        }


        if (isset($data->nome) AND ($data->nome)) {
            $filter = new TFilter('nome', 'ilike', "%{$data->nome}%"); // create the filter
            TSession::setValue('Vw40AtencaoMedicaList_filter_nome',   $filter); // stores the filter in the session
        }


        if (isset($data->mae) AND ($data->mae)) {
            $filter = new TFilter('mae', 'ilike', "%{$data->mae}%"); // create the filter
            TSession::setValue('Vw40AtencaoMedicaList_filter_mae',   $filter); // stores the filter in the session
        }


        if (isset($data->cns) AND ($data->cns)) {
            $filter = new TFilter('cns', 'ilike', "%{$data->cns}%"); // create the filter
            TSession::setValue('Vw40AtencaoMedicaList_filter_cns',   $filter); // stores the filter in the session
        }


        if (isset($data->idade) AND ($data->idade)) {
            $filter = new TFilter('idade', '=', "$data->idade"); // create the filter
            TSession::setValue('Vw40AtencaoMedicaList_filter_idade',   $filter); // stores the filter in the session
        }


        if (isset($data->qtd) AND ($data->qtd)) {
            $filter = new TFilter('qtd', 'ilike', "%{$data->qtd}%"); // create the filter
            TSession::setValue('Vw40AtencaoMedicaList_filter_qtd',   $filter); // stores the filter in the session
        }
        ...
        TSession::setValue('Vw40AtencaoMedicaPrimerissimaInfancia_filter_data', $data);
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for Vw40AtencaoMedicaPrimerissimaInfancia
            ...
            if (TSession::getValue('Vw40AtencaoMedicaList_filter_pessoa_id')) {
                $criteria->add(TSession::getValue('Vw40AtencaoMedicaList_filter_pessoa_id')); // add the session filter
            }


            if (TSession::getValue('Vw40AtencaoMedicaList_filter_nome')) {
                $criteria->add(TSession::getValue('Vw40AtencaoMedicaList_filter_nome')); // add the session filter
            }


            if (TSession::getValue('Vw40AtencaoMedicaList_filter_mae')) {
                $criteria->add(TSession::getValue('Vw40AtencaoMedicaList_filter_mae')); // add the session filter
            }


            if (TSession::getValue('Vw40AtencaoMedicaList_filter_cns')) {
                $criteria->add(TSession::getValue('Vw40AtencaoMedicaList_filter_cns')); // add the session filter
            }


            if (TSession::getValue('Vw40AtencaoMedicaList_filter_idade')) {
                $criteria->add(TSession::getValue('Vw40AtencaoMedicaList_filter_idade')); // add the session filter
            }


            if (TSession::getValue('Vw40AtencaoMedicaList_filter_qtd')) {
                $criteria->add(TSession::getValue('Vw40AtencaoMedicaList_filter_qtd')); // add the session filter
            }
