<?php
/**
 * Vw38SaudeBucalList Listing
 * @author  <your name here>
 */
class Vw38SaudeBucalList extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_Vw38SaudeBucalPrimerissimaInfancia');
        $this->form->setFormTitle('A38 - Crianças que precisam ir ao dentista');
        ...
        $this->form->setData( TSession::getValue('Vw38SaudeBucalPrimerissimaInfancia_filter_data') );
        
        // add the search form actions
        $btn = $this->form->addAction(_t('Find'), new TAction([$this, 'onSearch']), 'fa:search');
        $btn->class = 'btn btn-sm btn-primary';
        //$this->form->addActionLink(_t('New'), new TAction(['Vw38SaudeBucalPrimerissimaInfanciaForm', 'onEdit']), 'fa:plus green');
        ...
        // create EDIT action
        //$action_edit = new TDataGridAction(['Vw38SaudeBucalPrimerissimaInfanciaForm', 'onEdit']);
        ...
        $tipo='A38 - Crianças que precisam ir ao dentista';
    ...
    public function onInlineEdit($param)
    {
        try
        {
            ...
            $object = new Vw38SaudeBucalPrimerissimaInfancia($key); // instantiates the Active Record
            $object->{$field} = $value;
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('Vw38SaudeBucalList_filter_pessoa_id',   NULL);
        TSession::setValue('Vw38SaudeBucalList_filter_nome',   NULL);
        TSession::setValue('Vw38SaudeBucalList_filter_mae',   NULL);
        TSession::setValue('Vw38SaudeBucalList_filter_cns',   NULL);
        TSession::setValue('Vw38SaudeBucalList_filter_idade',   NULL);
        TSession::setValue('Vw38SaudeBucalList_filter_qtd',   NULL);

        if (isset($data->pessoa_id) AND ($data->pessoa_id)) {
            $filter = new TFilter('pessoa_id', '=', "$data->pessoa_id"); // create the filter
            TSession::setValue('Vw38SaudeBucalList_filter_pessoa_id',   $filter); // stores the filter in the session
        }


        if (isset($data->nome) AND ($data->nome)) {
            $filter = new TFilter('nome', 'ilike', "%{$data->nome}%"); // create the filter
            TSession::setValue('Vw38SaudeBucalList_filter_nome',   $filter); // stores the filter in the session
        }


        if (isset($data->mae) AND ($data->mae)) {
            $filter = new TFilter('mae', 'ilike', "%{$data->mae}%"); // create the filter
            TSession::setValue('Vw38SaudeBucalList_filter_mae',   $filter); // stores the filter in the session
        }


        if (isset($data->cns) AND ($data->cns)) {
            $filter = new TFilter('cns', 'ilike', "%{$data->cns}%"); // create the filter
            TSession::setValue('Vw38SaudeBucalList_filter_cns',   $filter); // stores the filter in the session
        }


        if (isset($data->idade) AND ($data->idade)) {
            $filter = new TFilter('idade', '=', "$data->idade"); // create the filter
            TSession::setValue('Vw38SaudeBucalList_filter_idade',   $filter); // stores the filter in the session
        }


        if (isset($data->qtd) AND ($data->qtd)) {
            $filter = new TFilter('qtd', 'ilike', "%{$data->qtd}%"); // create the filter
            TSession::setValue('Vw38SaudeBucalList_filter_qtd',   $filter); // stores the filter in the session
        }
        ...
        TSession::setValue('Vw38SaudeBucalPrimerissimaInfancia_filter_data', $data);
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for Vw38SaudeBucalPrimerissimaInfancia
            $repository = new TRepository('Vw38SaudeBucalPrimerissimaInfancia');
            $limit = 10;
            // creates a criteria
            ...
            if (TSession::getValue('Vw38SaudeBucalList_filter_pessoa_id')) {
                $criteria->add(TSession::getValue('Vw38SaudeBucalList_filter_pessoa_id')); // add the session filter
            }


            if (TSession::getValue('Vw38SaudeBucalList_filter_nome')) {
                $criteria->add(TSession::getValue('Vw38SaudeBucalList_filter_nome')); // add the session filter
            }


            if (TSession::getValue('Vw38SaudeBucalList_filter_mae')) {
                $criteria->add(TSession::getValue('Vw38SaudeBucalList_filter_mae')); // add the session filter
            }


            if (TSession::getValue('Vw38SaudeBucalList_filter_cns')) {
                $criteria->add(TSession::getValue('Vw38SaudeBucalList_filter_cns')); // add the session filter
            }


            if (TSession::getValue('Vw38SaudeBucalList_filter_idade')) {
                $criteria->add(TSession::getValue('Vw38SaudeBucalList_filter_idade')); // add the session filter
            }


            if (TSession::getValue('Vw38SaudeBucalList_filter_qtd')) {
                $criteria->add(TSession::getValue('Vw38SaudeBucalList_filter_qtd')); // add the session filter
            }
